...
(function() {
    ...
    function getNextBlockPosition( intersect ) {
        ...
        vector.divideScalar( BlockBuilder.BLOCK_SIZE ).floor().multiplyScalar( BlockBuilder.BLOCK_SIZE ).addScalar( BlockBuilder.BLOCK_SIZE * .5 );
        return vector;
    }
    ...
    function onCanvasClickWithoutMove( event ) {
        if( event.detail.button === 0 ) {
            ...
            if( intersects.length > 0 ) {
                ...
                var block = new THREE.Mesh( BlockBuilder.blockGeometry, new THREE.MeshLambertMaterial( { color: BlockBuilder.currentBlockColor } ) );
